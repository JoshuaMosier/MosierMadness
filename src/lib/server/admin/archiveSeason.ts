...
async function findOrCreatePerson(
  ...
  if (existing?.length === 0 && normalizedFirst && normalizedLast) {
    ...
    if (lastNameMatches?.length) {
      ...
      const fuzzyMatch = lastNameMatches.find((p: any) => {
  ...
  if (slugCheck && slugCheck.length > 0) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const existingSlugs = new Set(slugCheck.map((row: any) => row.slug));
 ...
 */
export async function archiveSeason(db: SupabaseClient, settings: TournamentSettings): Promise<ArchiveResult> {
  ...
  const personMap = new Map<string, string>(); // entryId -> personId

 ...
 *
 * @param {object} db - Authenticated Supabase client (must have admin permissions)
 * @param {{ dryRun?: boolean }} options
 */
export async function mergeDuplicatePeople(db, { dryRun = false } = {}) {
  // Load all people
  ...
  // Find merge candidates: same last name, first name prefix match,
  // one has linked_profile_id and the other doesn't
  const mergePairs = [];
  for (const [, group] of byLastName) {
    if (group.length < 2) continue;
    const linked = group.filter(p => p.linked_profile_id);
    const unlinked = group.filter(p => !p.linked_profile_id);
